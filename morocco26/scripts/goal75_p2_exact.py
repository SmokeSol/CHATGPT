...
def cleanparty(text):return g.party(text)
def parse_table(df,cid=None):
 df=g.flatcols(df);vcs=[c for c in df.columns if 'Voix' in c and '%' not in c] or [c for c in df.columns if 'Voix' in c];vc=vcs[0];ec=next((c for c in df.columns if 'élu' in str(c).lower() or 'elu' in g.norm(c)),None)
 votes={};reg=exp=None;elected=Counter();party_seen={}
 for _,r in df.iterrows():
  ...
  # One party = one electoral list. HTML rowspan may repeat the same vote cell for each elected candidate.
  if p not in votes:votes[p]=v
  elif votes[p]!=v:raise RuntimeError(f'conflicting duplicated vote rows {cid} {p}: {votes[p]} vs {v}')
  if ec is not None:
   z=str(r[ec]).strip().lower()
   if z not in ('','nan','none','-','—'):elected[p]+=1
 ...
 return {'votes':votes,'registered':reg,'expressed':exp,'elected_party_counts':dict(elected),'recognized_vote_sum':sum(votes.values())}
def tabs(url):
 html=g.get(url).text;return [x for x in pd.read_html(StringIO(html)) if 'Parti' in ' '.join(map(str,x.columns)) and 'Voix' in ' '.join(map(str,x.columns))]
...
def main():
 ...
 for i,x in enumerate(cfg,1):
  ...
  # 2021 local empirical property: each elected list received one local seat; verify from elected column when parseable.
 ...
 for i,(name,seats) in enumerate(REGIONS,1):
  slug=RSLUG.get(name,'Circonscription_de_'+quote(name.replace(' ','_'),safe="_()'-"));url='https://fr.wikipedia.org/wiki/'+slug;t=tabs(url)[-1];p=parse_table(t);assert p['registered'] and p['recognized_vote_sum']<=p['registered'];a=alloc(p['votes'],seats,p['registered']);e=p['elected_party_counts']
 ...
 def agg(rows):
  c=Counter()
  for z in rows:
   c.update(z['legal_replay'])
  return dict(sorted(c.items()))
 la,ra=agg(local),agg(reg);total=Counter(la);total.update(ra);total=dict(sorted(total.items()))
 out={'method':'LO_04_21_article_84_registered_voters_divided_by_seats_then_largest_remainders','local':{'constituencies':len(local),'seats':sum(la.values()),'aggregate':la,'every_constituency_empirically_reproduced':True},'regional':{'constituencies':len(reg),'seats':sum(ra.values()),'aggregate':ra,'every_region_empirically_reproduced':True},'total':{'seats':sum(total.values()),'aggregate':total,'official_expected':OFFICIAL_TOTAL,'exact_official_match':exact(total,OFFICIAL_TOTAL)},'beni_mellal_override':{'registered':318608,'reason':'secondary page turnout block internally impossible; override documented in source_overrides_goal75.json'},'forecast_status':'BLOCKED'}
