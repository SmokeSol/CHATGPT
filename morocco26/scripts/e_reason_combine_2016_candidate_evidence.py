...
deduplicated within territory by party + normalized Arabic name. A district
...
def main():
 ...
 allowed_status={'PASS','PARTIAL_VALID','FAIL_CLOSED_DIAGNOSTIC_PERSISTED'}
 if pjd.get('status') not in allowed_status:raise RuntimeError('unexpected PJD parser status')
 if pps.get('status') not in {'PASS','PARTIAL_VALID'}:raise RuntimeError('PPS parser is not a usable valid subset')
 all_rows=[]
 for source,payload in [('PJD',pjd),('PPS',pps)]:
  for c in payload.get('candidate_rows',[]):
   # Only rows actually promoted by each fail-closed parser are present here.
   name=c.get('candidate_name_ar_normalized') or ''
   cid=c.get('constituency_id')
   if not cid or not name:continue
   all_rows.append({**c,'evidence_pipeline':source})
 by=defaultdict(dict)
 for c in all_rows:
  key=(c.get('party'),c['candidate_name_ar_normalized'])
 ...
 for cid,items in sorted(by.items()):
  ...
  district_rows.append({'constituency_id':cid,'verified_distinct_candidate_identities':identity_count,'identity_coverage_pass':identity_count>=3,'enriched_candidate_fact_present':bool(enriched),'parties':sorted({str(x.get('party')) for x in xs}),'pipelines':sorted({x['evidence_pipeline'] for x in xs}),'candidate_keys':[{'party':x.get('party'),'name_ar_normalized':x['candidate_name_ar_normalized'],'pipeline':x['evidence_pipeline']} for x in xs]})
