...
EXPECTED_RNI_REGIONAL=2120854

def norm(x):return re.sub('[^a-z0-9]+',' ',unicodedata.normalize('NFKD',str(x)).encode('ascii','ignore').decode().lower()).strip()
def main():
 r=requests.get(URL,timeout=60,headers={'User-Agent':'MOROCCO26 research'});r.raise_for_status();book=pd.ExcelFile(BytesIO(r.content));summary={'source_url':URL,'bytes':len(r.content),'sheets':book.sheet_names,'sheet_profiles':[],'marrakech_matches':[],'regional_rows':[]}
 for s in book.sheet_names:
  ...
  if s=='donnees':
   ...
   reg=df[df['typeListe'].astype(str).str.lower().eq('regionale')].copy()
   ...
   totals=Counter()
   for rr in summary['regional_rows']:totals.update(rr['votes'])
   summary['regional_party_vote_totals']={k:int(v) for k,v in sorted(totals.items())}
   ...
   summary['RNI_ministry_expected']=EXPECTED_RNI_REGIONAL;summary['RNI_tafra_sum']=int(totals['RNI']);summary['RNI_exact_match']=int(totals['RNI'])==EXPECTED_RNI_REGIONAL
 ...
 summary['regional_source_gate_pass']=summary.get('regional_row_count')==12 and summary.get('regional_seat_sum')==90 and summary.get('RNI_exact_match') is True
 (O/'tafra_legislative_workbook_audit.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2));print(json.dumps({'regional_row_count':summary.get('regional_row_count'),'regional_seat_sum':summary.get('regional_seat_sum'),'RNI_tafra_sum':summary.get('RNI_tafra_sum'),'RNI_ministry_expected':EXPECTED_RNI_REGIONAL,'RNI_exact_match':summary.get('RNI_exact_match'),'gate':summary['regional_source_gate_pass'],'regional_rows':summary['regional_rows']},ensure_ascii=False,indent=2));raise SystemExit(0 if summary['regional_source_gate_pass'] else 8)
