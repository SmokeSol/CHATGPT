...
outcome has not yet been ingested. It does not read any same-year result workbook,
canonical outcome, post-election elected-member list, or outcome-derived map.
...
# Safety: these paths must not exist on the clean lineage and are never inputs.
FORBIDDEN = [
    ...
    ROOT / "data" / "goal100" / "older_history_probe" / "raw" / "parlement-elections-2007-1-0.xlsx",
]

# Sources already independently discovered as contemporaneous.  The crawler verifies
# their publication date and stores the text it actually observed; it does not trust
# this seed list as data.
SEED_URLS = [
...
# Broad, outcome-independent discovery queries.  They search the contemporary corpus,
# not a checklist learned from results.
DISCOVERY_QUERIES = [
...
# High-precision patterns only. We deliberately prefer missingness to hallucinated
...
def fetch_article(url: str) -> dict:
    ...
    if not date:
        rec["status"] = "AMBIGUOUS_DATE"
        rec["text_head"] = text[:800]
        return rec
    if date > CUTOFF:
        rec["status"] = "POST_CUTOFF_REJECTED"
        return rec
    if date < "2007-01-01":
        rec["status"] = "WRONG_YEAR_REJECTED"
        return rec
    rec["status"] = "ELIGIBLE_PRE_ELECTION"
    rec["text"] = text
    return rec
    ...
    for pat in EXPLICIT_PATTERNS:
        for m in pat.finditer(text):
            ...
            try:
                ...
                continue
            if not (2 <= seats <= 6) or len(name) > 80:
                continue
            # Reject obviously generic/national captures.
            low = name.lower()
            if any(x in low for x in ("maroc", "chambre", "liste nationale", "parlement", "province compte", "région compte")):
                continue
            ...
            facts.append({
                "fact_type": "DISTRICT_MAGNITUDE_EXPLICIT",
                "territory_as_published": name,
                "magnitude": seats,
                "publication_date": article["publication_date"],
                "url": article["url"],
                "source_title": article.get("title"),
                "supporting_context": text[lo:hi],
...
def assert_clean_lineage() -> dict:
    ...
    if present:
        raise SystemExit(f"LEAKAGE_GUARD_FAIL forbidden 2007 outcome paths present: {present}")
    # Keep a machine-readable ancestry proof.  The expected clean parent is the 2002
    # outcome commit immediately before the first 2007 snapshot in the original line.
    ...
    if not ok:
        raise SystemExit("LEAKAGE_GUARD_FAIL clean base is not ancestor")
    return {"head_at_run": head, "clean_base": base, "forbidden_paths_present": present, "base_is_ancestor": True}
...
def main() -> None:
    ...
    for q in DISCOVERY_QUERIES:
        urls = search_ddg(q)
        search_log.append({"query": q, "urls": urls})
        for u in urls:
            if u not in discovered:
                discovered.append(u)
        time.sleep(0.4)

    # Limit bounded crawl; deterministic URL order.
    ...
    cert = {
        "assertion": "TARGET_OUTCOME_NOT_USED_IN_PRE_ELECTION_RECOVERY_V2",
        "cutoff": CUTOFF,
        "same_year_outcome_paths_present": False,
        "same_year_result_workbook_opened": False,
        "clean_base_commit": lineage["clean_base"],
        "status": "PASS_RECOVERY_RESEARCH_ONLY",
    }
