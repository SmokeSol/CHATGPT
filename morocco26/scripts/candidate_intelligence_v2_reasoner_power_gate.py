...
A21_RESOLUTION=CI/'2021'/'pjd_prior_mp_identity_resolution_v2.json'
...
def eval_transition(label, paths, req):
 ...
 if label=='2016_TO_2021':
  resolution=json.loads(A21_RESOLUTION.read_text(encoding='utf-8'))
  for d in resolution['decisions']:
   tid=d['territory_id']
   if tid not in A: raise RuntimeError(f'resolution territory missing from A: {tid}')
   if A[tid].get('feature_state')!='UNKNOWN': raise RuntimeError(f'resolution may only replace UNKNOWN, got {A[tid].get("feature_state")} for {tid}')
   A[tid]={**A[tid], 'feature_state':d['state'], 'final_identity_resolution':d}
   applied.append({'territory_id':tid,'state':d['state'],'method':d['method']})
 ...
 return {
  ...
  'A_identity_resolutions_applied':applied,'gates':gates,'gate_pass':all(gates.values())}
...
def main():
 ...
 out={
  'schema_version':'1.1','gate_id':'M26-CANDIDATE-INTELLIGENCE-V2-REASONER-POWER-GATE-V1','contract_id':c['contract_id'],'contract_frozen_at':c['frozen_at'],
  ...
  'A21_identity_resolution_artifact':str(A21_RESOLUTION.relative_to(ROOT)),
