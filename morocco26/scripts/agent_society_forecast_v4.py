...
from morocco26.agent_society_v4.main_adapter import GitSnapshotReader,source_inventory,candidate_records,program_records,territory_records,registered_totals
...
def rows(value):
    ...
    raise ValueError("expected array or rows/cells/records/contests object")
...
def main(argv=None):
    ...
    for name in ("inventory-main","adapt-candidates","adapt-programs","adapt-territories","adapt-registered-electorate"):
        p=sub.add_parser(name); p.add_argument("--repo-root",type=pathlib.Path,default=ROOT); p.add_argument("--main-ref",required=True); p.add_argument("--output",required=True)
        if name in {"adapt-candidates","adapt-programs"}: p.add_argument("--as-of",required=True)
    ...
    elif args.cmd=="adapt-registered-electorate": rec,un=registered_totals(reader); write(args.output,{"main_commit_sha":reader.commit_sha,"registered_totals":rec,"unresolved":un})
    ...
    elif args.cmd=="calibrate-electorate":
        totals_value=load(args.registered_totals); totals=totals_value.get("registered_totals",totals_value) if isinstance(totals_value,dict) else totals_value; write(args.output,{"cells":calibrate_to_registered_totals(rows(load(args.cells)),totals)})
