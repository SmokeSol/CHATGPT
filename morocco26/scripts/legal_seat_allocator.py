...
def _assert_full_list_universe(votes: Mapping[str, float]) -> None:
    ...
    if bad:
        raise AllocationError(
            "Exact legal allocation rejects aggregate OTHER/AUTRES buckets; "
            "provide each competing list separately."
        )

def allocate_from_shares(
    shares_by_list: Mapping[str, float], seats: int, year: int, tier: str = "local"
) -> dict[str, Any]:
    registry = load_registry()
    rule = registry["regimes"][str(year)][tier]
    if rule["quotient_mode"] == "registered_voters_div_seats":
        raise AllocationError(
            f"{year} {tier}: shares alone are insufficient because the quotient "
            "depends on registered voters; absolute votes are required."
        )
    shares = _clean_votes(shares_by_list)
    total = sum(shares.values())
    votes = {k: v / total for k, v in shares.items()}
    return allocate(votes, seats, year, tier=tier, registered_voters=None)

def allocate(
    votes_by_list: Mapping[str, float],
    seats: int,
    year: int,
    *,
    tier: str = "local",
    registered_voters: float | None = None,
    strict_list_universe: bool = True,
) -> dict[str, Any]:
    if seats <= 0:
        raise AllocationError("seats must be positive")
    votes = _clean_votes(votes_by_list)
    if strict_list_universe:
        _assert_full_list_universe(votes)
    registry = load_registry()
    try:
        rule = registry["regimes"][str(year)][tier]
    except KeyError as exc:
        raise AllocationError(f"unsupported regime/tier: {year}/{tier}") from exc
    ...
    if segment_counts and seats not in set(int(x) for x in segment_counts):
        raise AllocationError(
            f"{year} {tier}: exact allocation is defined independently for seat segments "
            f"{segment_counts}; call the allocator once per segment rather than pooling them."
        )
    ...
    if not eligible:
        return {
            ...
            "eligible_lists": [], "excluded_lists": excluded,
            "quotient": None, "status": "NO_ELIGIBLE_LIST",
    ...
    for k, v in eligible.items():
        initial = int(math.floor((v + 1e-12) / quotient))
        seat_map[k] = initial
        remainders[k] = v - initial * quotient

    allocated = sum(seat_map.values())
    if allocated > seats:
        raise AllocationError("initial quotient allocation exceeds district magnitude")
    remaining = seats - allocated
    ranked = sorted(remainders.items(), key=lambda kv: (-kv[1], kv[0]))
    tie_groups: list[list[str]] = []
    i = 0
    while i < len(ranked):
        j = i + 1
        while j < len(ranked) and abs(ranked[j][1] - ranked[i][1]) <= 1e-12:
            j += 1
        if j - i > 1:
    ...
    for pos, (k, rem) in enumerate(ranked):
        if awarded >= remaining:
            break
        if pos + 1 == remaining and pos + 1 < len(ranked):
            if abs(rem - ranked[pos+1][1]) <= 1e-12:
                cutoff_tie = [x for x,r in ranked if abs(r-rem) <= 1e-12]
                break
        seat_map[k] += 1
        awarded += 1

    allocated = sum(seat_map.values())
    unallocated = seats - allocated
    status = "ALLOCATED"
    if cutoff_tie:
        status = "UNRESOLVED_LEGAL_TIE"
    elif unallocated:
        status = "UNALLOCATED_LEGAL_EDGE_CASE"

    return {
        "year": year, "tier": tier, "seats_requested": seats,
        "seats_allocated": allocated, "seats": seat_map,
        "eligible_lists": sorted(eligible), "excluded_lists": excluded,
        "threshold_fraction_valid_votes": threshold,
        "quotient_mode": mode, "quotient": quotient,
        "remainders": remainders, "tie_groups": tie_groups,
        "cutoff_tie": cutoff_tie, "unallocated_seats": unallocated,
        "status": status
    }

def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--year", type=int, required=True, choices=[2002,2007,2011,2016,2021,2026])
    ap.add_argument("--tier", default="local", choices=["local","national","regional"])
    ap.add_argument("--seats", type=int, required=True)
