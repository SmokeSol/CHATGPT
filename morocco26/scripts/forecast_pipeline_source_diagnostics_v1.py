...
OUT_2011 = FP / "diagnostic_2011_raw_party_universe_v2.json"
...
def audit_2011() -> dict[str, Any]:
    ...
    for ws in wb.worksheets:
        ...
        if ws.title.casefold() in {"dictionnaire", "notes"}:
            full_nonempty_rows[ws.title] = full_rows
    ...
    idx = {str(v): i for i, v in enumerate(header) if v is not None}
    party_cols = [k for k in canonical_keys if k in idx]
    aggregate_votes = {k: 0.0 for k in party_cols}
    reconstruction_residuals = []
    local_rows = 0
    for vals in ws.iter_rows(min_row=2, values_only=True):
        kind = str(vals[idx["typeListe"]]).strip().casefold() if "typeListe" in idx and vals[idx["typeListe"]] is not None else ""
        if kind not in {"locale", "local"}:
            continue
        local_rows += 1
        party_sum = 0.0
        for k in party_cols:
            v = float(vals[idx[k]] or 0)
            aggregate_votes[k] += v
            party_sum += v
        registered = float(vals[idx["nInscrits"]] or 0) if "nInscrits" in idx else 0.0
        turnout = float(vals[idx["txParticipation"]] or 0) if "txParticipation" in idx else 0.0
        invalid = float(vals[idx["pctNuls"]] or 0) if "pctNuls" in idx else 0.0
        if registered > 0 and turnout >= 0 and 0 <= invalid < 1:
            implied_valid = registered * turnout * (1.0 - invalid)
            reconstruction_residuals.append(implied_valid - party_sum)

    max_abs_resid = max((abs(x) for x in reconstruction_residuals), default=None)
    mean_abs_resid = (
        sum(abs(x) for x in reconstruction_residuals) / len(reconstruction_residuals)
        if reconstruction_residuals else None
    )

    target_present = {k: bool(v) for k, v in target_hits.items()}
    if any(target_present.values()):
        status = "INGESTION_OR_CODE_MAPPING_REVIEW_REQUIRED"
        interpretation = (
            "At least one externally named missing-winner party is present somewhere in the raw workbook; "
            "the canonical mapping must be reconciled before blaming source coverage."
        )
    elif max_abs_resid is not None and max_abs_resid < 1e-6:
        status = "SELF_CONTAINED_19_CODE_UNIVERSE_REQUIRES_CODEBOOK_RECONCILIATION"
        interpretation = (
            "The retained raw workbook has only the 19 canonical party columns and contains no literal "
            "PLJS/PRE/PUD labels, yet its party-column sums exactly reconstruct its own valid-vote identity. "
            "This means 'missing party votes' cannot be asserted from the workbook alone. Reconcile the "
            "TAFRA codebook/party universe with the independent 2011 seat reference before altering votes."
        )
    else:
        status = "SOURCE_COVERAGE_OR_DERIVATION_AMBIGUOUS"
        interpretation = (
            "The raw workbook lacks the external party labels and its internal totals do not prove a closed "
            "vote universe. Do not patch from seat outcomes; obtain a complete source/codebook."
        )

    return {
        "schema_version": "1.1",
        "diagnostic_id": "M26-2011-RAW-PARTY-UNIVERSE-DIAGNOSTIC-V2",
        ...
        "raw_party_columns": party_cols,
        "raw_party_aggregate_votes_local": aggregate_votes,
        "raw_local_rows": local_rows,
        "workbook_sheets": sheets,
        "first_nonempty_rows_preview": previews,
        "dictionary_and_notes_full_nonempty_rows": full_nonempty_rows,
        ...
        "valid_vote_identity_check": {
            "rows_checked": len(reconstruction_residuals),
            "max_abs_implied_valid_minus_party_sum": max_abs_resid,
            "mean_abs_implied_valid_minus_party_sum": mean_abs_resid,
            "warning": "An exact identity may reflect how turnout/null-rate fields were constructed in the source; it is not by itself proof that no party was omitted."
        },
        "status": status,
...
def probe_2021() -> dict[str, Any]:
    ...
    return {
        ...
        "status": ("OFFICIAL_SURFACE_DISCOVERED_NEEDS_ENDPOINT_EXTRACTION" if candidate_pages else "NO_MACHINE_READABLE_DISTRICT_COUNTS_FOUND_ON_PROBED_OFFICIAL_SURFACE"),
...
def main():
    ...
    print(json.dumps({"2011_status": a["status"], "2011_target_present": a["target_present"], "2021_status": b["status"], "2021_pages": len(b["pages_fetched"]), "2021_candidate_pages": len(b["candidate_pages_with_electorate_terms"])}, sort_keys=True))
