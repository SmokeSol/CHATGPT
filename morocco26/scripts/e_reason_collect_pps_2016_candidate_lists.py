...
The domain is pre-qualified as T1_OFFICIAL_PARTY. Discovery is bounded to the
PPS candidate category and 2-6 Oct 2016 publication window. Post-election pages
are excluded by construction. Current migrated pages are discovery/provenance;
static candidate documents are fingerprinted and admitted only when their first-
party page publication date is pre-cutoff and the document contains prospective
2016 legislative-candidate content without outcome language.
"""
...
def qualified():
 d=json.loads(REG.read_text(encoding='utf-8')); return any(x.get('domain')=='pps.ma' and x.get('source_class')=='T1_OFFICIAL_PARTY' and x.get('qualification_status')=='QUALIFIED_BEFORE_EXTRACTION' for x in d.get('entries',[]))
...
def clean_text(raw):
 return ' '.join(BeautifulSoup(html.unescape(raw or ''),'html.parser').get_text(' ',strip=True).split())
def parse_dt(value):
...
def extract_urls(text,base):
 soup=BeautifulSoup(html.unescape(text.replace('\\/','/')),'html.parser'); out=set()
 ...
 return sorted(u for u in out if (urlparse(u).hostname or '').lower().removeprefix('www.')=='pps.ma')
...
def inspect_pdf(body,h):
 reader=PdfReader(BytesIO(body)); pages=[]
 for i,p in enumerate(reader.pages):pages.append(f'--- PAGE {i+1} ---\n'+(p.extract_text() or ''))
 text='\n'.join(pages); tp=TEXT/(h+'.txt'); tp.write_text(text,encoding='utf-8')
 meta={str(k):str(v) for k,v in (reader.metadata or {}).items()}
 return {'pages':len(reader.pages),'metadata':meta,'text_path':str(tp.relative_to(ROOT)),'text_chars':len(text),'pre_terms':[x for x in PRE_TERMS if x in text],'outcome_terms':[x for x in OUTCOME_TERMS if x in text]}

def main():
 if not qualified():raise RuntimeError('pps.ma must be T1-qualified before extraction')
 fetches=[]; discovered=set(); candidate_pages={}
 queue=list(SEEDS); seen=set()
 while queue and len(seen)<180:
  ...
  try:
   r=get(u); body=r.content; ct=(r.headers.get('content-type') or '').lower(); suffix='.json' if 'json' in ct else ('.xml' if 'xml' in ct else '.html')
   h,p=freeze(body,suffix); text=body.decode('utf-8',errors='replace')
 ...
 for u in sorted(discovered):
  rec={'url':u,'status':None,'bytes':0,'content_type':None,'sha256':None,'path':None,'pdf':None,'error':None,'source_class':'T1_OFFICIAL_PARTY','transport':'CURRENT_FIRST_PARTY_STATIC_DISCOVERY'}
  try:
   r=get(u,attempts=3,timeout=(7,45)); body=r.content; ct=(r.headers.get('content-type') or '').lower(); ext=Path(urlparse(str(r.url)).path).suffix.lower() or '.bin'
   if len(ext)>8:ext='.bin'
   h,p=freeze(body,ext); rec.update(status=r.status_code,bytes=len(body),content_type=ct,sha256=h,path=p)
