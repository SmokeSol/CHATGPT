...
def fast_choose_population(demo,pol,seed):
    ...
    for attempt in range(4):
        rows=base.make_support(pol,seed,attempt); w,err=fast_rake(rows,targets)
    ...
    if best is None:
        raise RuntimeError('no 128-archetype support among four frozen deterministic candidates satisfies all numerical gates')
    dep,negess,mw,attempt,rows,w,err=best
    for i,(r,x) in enumerate(zip(rows,w),start=1): r['archetype_id']=f'A{i:03d}'; r['weight']=float(x)
    return rows,{'raking_max_abs_error':float(err),'effective_archetype_count':float(-negess),'max_single_archetype_weight':mw,'max_demo_prior_joint_deviation_from_independence':float(dep),'support_attempt':attempt,'ipf_implementation':'PRECOMPUTED_CATEGORY_INDEX_V3_GATE_ALIGNED'}
