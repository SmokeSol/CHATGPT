...
def main():
    ...
    bundle={**power,'schema_version':'terminal-bundle-1.0','result_id':'M26-CANDIDATE-INTELLIGENCE-V2-MULTIPART-TERMINAL-BUNDLE-V1','power_gate_v4':power,'predictive_gate_v2':score,'terminal_certificate_v2':terminal,'batch_terminal_status':terminal['terminal_status'],'batch_terminal_decision':terminal['terminal_decision']}
    legacy_power.write_text(json.dumps(bundle,ensure_ascii=False,indent=2,sort_keys=True)+'\n',encoding='utf-8')

    # TEMPORARY observability hook: execute the already-frozen F1 conventional baseline
    # preflight on the same known runner. This does not modify any F1 rule or threshold.
    f1=load_module('f1baseline',HERE/'goal100_f1_baseline_rebuild_v1.py')
    f1.main()
    f1_result=json.loads(f1.OUT_DEV.read_text(encoding='utf-8'))
