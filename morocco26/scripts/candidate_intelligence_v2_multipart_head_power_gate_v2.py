...
def main():
    # Preserve the existing deterministic Candidate Intelligence terminal pipeline.
    ...
    ci=v4.CI; legacy_power=ci/'candidate_intelligence_v2_multipart_head_power_gate_v1.json';legacy16=ci/'multipart'/'2016_head_prior_mp_features_v1.jsonl';legacy21=ci/'multipart'/'2021_head_prior_mp_features_v1.jsonl';legacy_excluded=ci/'multipart'/'2021_unresolved_territory_heads_excluded_v1.json'
    shutil.copyfile(v4.D16,legacy16);shutil.copyfile(v4.D21,legacy21);dedup=json.loads(v4.DEDUP.read_text(encoding='utf-8'))
    ...
    # TEMPORARY batch observability hooks for already-frozen F1 conventional baseline audits.
    f1=load_module('f1baseline',HERE/'goal100_f1_baseline_rebuild_v1.py'); f1.main(); f1_result=json.loads(f1.OUT_DEV.read_text(encoding='utf-8'))
    print('F1_OBSERVABLE_RESULT='+json.dumps({'status':f1_result['status'],'eligible_candidate_count':f1_result['eligible_candidate_count'],'selected_candidate':f1_result['selected_candidate']},ensure_ascii=False,sort_keys=True))
