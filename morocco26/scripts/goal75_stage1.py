...
S=requests.Session();S.headers['User-Agent']='MOROCCO26-research/1.0'
P=['RNI','PAM','PI','PJD','USFP','MP','UC','PPS']; K=P+['OTHER']
...
def resolve(name):
 q={'action':'query','list':'search','srsearch':f'intitle:Circonscription "{name}"','format':'json','srlimit':10,'utf8':1};a=S.get('https://fr.wikipedia.org/w/api.php',params=q,timeout=25).json()['query']['search']
 if not a:q['srsearch']=f'Circonscription {name}';a=S.get('https://fr.wikipedia.org/w/api.php',params=q,timeout=25).json()['query']['search']
 if not a:raise RuntimeError('no page')
 target=set(norm(name).split());title=max(a,key=lambda z:len(target&set(norm(z['title']).split())))['title'];return title,'https://fr.wikipedia.org/wiki/'+quote(title.replace(' ','_'),safe="_()'-")
def flatcols(df):
...
def acquire(name,seats,need21):
 title,url=resolve(name);html=S.get(url,timeout=30).text;tabs=[]
 for df in pd.read_html(StringIO(html)):
  cols=' '.join(map(str,df.columns))
  if 'Parti' in cols and 'Voix' in cols:tabs.append(df)
 if not tabs:raise RuntimeError('no election tables')
...
def freeze(dev,hold):
 ...
 for x in dev:
  ...
  for k in K:d=c2[k]-c1[k];sh[k].append(d);rh[x['region']][k].append(d)
  if a['registered'] and b['registered']:d=lg(b['expressed']/b['registered'])-lg(a['expressed']/a['registered']);td.append(d);rtd[x['region']].append(d)
...
def main():
 ...
 for i,r in enumerate(rows,1):
  try:
   ...
   print(i,r['name'],'ok',flush=True);time.sleep(.05)
  except Exception as e:err.append({'name':r['name'],'holdout':norm(r['name']) in hn,'error':repr(e)});print(i,r['name'],'ERROR',e,flush=True)
 f=freeze(dev,hi);m=[]
 for x in dev:
  y=x['2021'];rr=sorted(y['votes'].items(),key=lambda z:(-z[1],z[0]));
  if len(rr)>x['seats']:a,b=rr[x['seats']-1],rr[x['seats']];m.append({'constituency_id':x['constituency_id'],'name':x['name'],'region':x['region'],'seats':x['seats'],'last_winner':a[0],'last_winner_votes':a[1],'first_nonwinner':b[0],'first_nonwinner_votes':b[1],'margin_votes':a[1]-b[1],'registered':y['registered'],'source_url':x['source_url']})
 ...
 ev=[x for x in dev if x.get('legal_replay')];ok=sum(x['legal_vs_rank_match'] for x in ev);rep={'development_target':80,'development_acquired':len(dev),'holdout_inputs_2016_only':len(hi),'holdout_2021_outcomes_accessed':False,'errors':err,'legal_replay_evaluable':len(ev),'legal_vs_rank_matches':ok,'seat_margin_records':len(m),'bc_freeze_hash':f['freeze_hash'],'ready_to_unseal':len(dev)>=75 and len(hi)==12 and len(ev)>=70 and ok/max(1,len(ev))>=.95};(O/'stage1_report.json').write_text(json.dumps(rep,ensure_ascii=False,indent=2));print(json.dumps(rep,ensure_ascii=False,indent=2));return 0 if rep['ready_to_unseal'] else 2
