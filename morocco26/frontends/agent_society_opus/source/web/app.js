...
function boot() {
  ...
  forces(); sanction(); bascule(); ecoute(); portraits(); atlas(); rules();
  reveal(); spy();
}
...
function header() {
  var g = S.global['*'];
  $('#m-rows').textContent = nf(S.meta.rows);
  $('#m-terr').textContent = nf(S.meta.territoires);
  $('#hero-fid').textContent = pct(g.fid, 1);
  $('#hero-part').textContent = pct(g.part, 1);
  $('#hero-sanc').textContent = pct(g.sanction, 1);
  $('#foot-hash').textContent = 'prompt a2561eb0…c8f71b6 · schéma fc4d26be…7c96b74f · ' +
    nf(S.meta.rows) + ' décisions · ' + nf(S.meta.work_items) + ' lots';
}

function kpis() {
  var g = S.global['*'], t = S.territoires;
  var lo = t[t.length - 1], hi = t[0];
  var K = [
    [nf(S.meta.rows), 'décisions gelées, une par agent et par lecture'],
    [nf(47104), 'profils statistiques distincts'],
    [nf(S.meta.territoires), 'territoires anonymes, 512 décisions chacun'],
    [pct(g.partant, 0), 'des anciens électeurs sont en voie de départ'],
    [pct(hi.part, 0) + ' → ' + pct(lo.part, 0), 'amplitude de participation entre territoires'],
    [pct(g.programme, 0), 'des décisions citent une attente programmatique']
  ];
  var host = $('#kpis');
  clear(host);
...
function steps() {
  ...
  var ST = [
    ...
    ['Il décide, avec sa part de doute', 'Une probabilité d’aller voter, une répartition entre les neuf listes, et onze parts qui expliquent d’où vient le résultat.']
  ];
  var host = $('#steps');
  clear(host);
  ...
  var n = $('#mec-note');
  clear(n);
  n.appendChild(el('b', null, 'Ce que le modèle refuse de faire : '));
  n.appendChild(document.createTextNode(
    'deviner une religion, une langue, une origine ou une proximité partisane ; ' +
    'traiter une donnée manquante comme un indice ; faire basculer un agent de façon spectaculaire ' +
    'sur une information non vérifiée ; ou utiliser la réponse d’un autre agent. ' +
    'Aucun agent ne parle à un autre : c’est justement l’objet de la version suivante.'));
}
...
function simulator() {
  ...
  SIM.ctx.parties.forEach(function (q, i) {
    ...
    o.value = q;
    o.textContent = 'Voté pour la liste ' + q.replace('Q_', '') + ' — ' +
      (SIM.ctx.gov[i] === 'INCUMBENT_COALITION' ? 'coalition sortante'
        : SIM.ctx.gov[i] === 'OPPOSITION' ? 'opposition' : 'ensemble résiduel');
    sel.appendChild(o);
  });
  sel.value = CTL.prior;
  ...
  var foot = el('p', 'sub', 'Territoire ' + SIM.ctx.territoire + ' du corpus · participation précédente ' +
    pct(SIM.ctx.prevturnout, 1) + ' · ' + SIM.ctx.ncomp + ' profils concurrents vérifiés.');
  ...
  function render() {
    ...
    order.forEach(function (o) {
      ...
      barRow(hp, 'Liste ' + o[0].replace('Q_', '') + ' · ' + tag, d.pp[i] / 0.6,
    ...
    v.appendChild(document.createTextNode('Cet agent a '));
...
function drawForces() {
  ...
  if (forcesDim === null) {
    ...
    $('#forces-note').textContent = 'Pour ' + pct(d2.social_protection_and_public_services / tot, 0) +
      ' des agents, ce qui départage vraiment les listes, une fois l’inertie mise de côté, ' +
...
function drawSancTbl() {
  ...
  ['Population', 'Agents', 'Sanctionnent', 'Récompensent', 'Participation', 'Fidélité'].forEach(function (h) {
...
function bascule() {
  ...
  $('#bascule-note').textContent =
    'Sur ' + nf(g.fidn) + ' agents ayant déjà voté, un sur cinq seulement est réellement acquis. ' +
...
function drawDetail(a) {
  ...
  card.appendChild(el('div', 'eyebrow', 'Décision ' + a[K.id] + ' · territoire ' + a[K.t] + ' · lecture ' + a[K.c]));
  ...
  var rows = [
    ...
    ['La fois précédente', a[K.pr] === 'ABSTAIN' ? 'Ne s’est pas déplacé' : 'A voté pour la liste ' + a[K.pr].replace('Q_', '')]
  ];
...
function narrate(a) {
  ...
  t.push(a[K.pr] === 'ABSTAIN'
    ...
    : 'Ce profil avait voté pour la liste ' + a[K.pr].replace('Q_', '') + ' ; le modèle lui donne ' + pct(a[K.part], 0) +
...
function drawAtlasStats() {
  ...
  var CARDS = [
    ...
    ['Fidélité la plus fragile', pct(Math.min.apply(null, T.map(function (t) { return t.fid; })), 0),
      'Dans le territoire le moins stable, l’agent moyen ne garde qu’une part minoritaire sur sa liste précédente.']
  ];
...
function rules() {
  ...
  var R = [
    ...
    ['Anonymat', 'Ni parti, ni territoire, ni date',
      'Les listes portent des pseudonymes valables uniquement à l’intérieur d’un territoire : la « liste 3 » d’un territoire n’a rien à voir avec la « liste 3 » d’un autre. Aucune identification n’a été tentée.'],
    ['Profils synthétiques', 'Personne n’est représenté',
      'Chaque agent est un profil statistique reconstruit à partir de données agrégées et anonymisées, pas une personne. Aucune caractéristique sensible n’est déduite : ni religion, ni langue, ni origine, ni proximité partisane.'],
    ...
    ['Agents isolés', 'Aucune influence entre eux',
      'Aucun agent ne voit la réponse d’un autre. Ni famille, ni collègues, ni voisinage, ni réseau. C’est une limite assumée de cette version, et l’objet de la suivante.']
  ];
