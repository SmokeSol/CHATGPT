...
function claimContribution(provider) {
  ...
  fetch(READER_API+'/claim',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({provider:provider})}).then(function(r){return r.json().then(function(j){if(!r.ok)throw j;return j;});}).then(function(claim){
    ...
    var prompt=contributionPrompt(claim.payload);
    return copyText(prompt).then(function(){showContributionReady(provider);});
...
function publiciseRenderedLabels() {
  ...
  document.querySelectorAll('#detail, #verdict-card, #demonstrateur').forEach(function(root){
    ...
    while((n=walker.nextNode())){if(!n.nodeValue||n.parentNode&&/SCRIPT|STYLE/.test(n.parentNode.nodeName))continue;n.nodeValue=n.nodeValue.replace(/\bliste\s+([1-9])\b/gi,function(_,d){return map[d]||_;});}
  });
}

function readerBoot() {
  loadMaroc(); readerMethod(); readerKpis(); refreshProgress(); setInterval(refreshProgress,60000);
  var join=reader$('#join-society'); if(join)join.addEventListener('click',assistantChooser);
  setTimeout(function(){readerKpis();refreshProgress();readerMethod();publiciseRenderedLabels();},800);
