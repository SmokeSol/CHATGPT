...
function editionDateFr(value){
  if(!value)return'—';
  const raw=String(value).trim();
  const iso=(raw.match(/\d{4}-\d{2}-\d{2}/)||[])[0]||raw;
...
function renderHeader(){
  ...
  const editionKey=ec?.edition_date||ec?.edition_id;
  $('#edition-date').textContent=editionKey?editionDateFr(editionKey):dateFr(s.created_at);
  ...
  const title=editionKey?`Édition du ${editionDateFr(editionKey)}`:'Édition quotidienne';
...
function renderHistory(){
  ...
  const cards=rows.slice(0,12).map((r,i)=>{const title=r.projection_changed?'Projection actualisée':'Projection stable';const when=editionDateFr(r.edition_date||r.edition_id);return`<article class="history-card-premium ${i===0?'current':''}"><span class="date">${when}</span><h3>${title}</h3><p>${esc(r.summary_fr||'Édition quotidienne enregistrée.')}</p><span class="status">${i===0?'Édition actuelle':'Édition conservée'}</span></article>`}).join('');
