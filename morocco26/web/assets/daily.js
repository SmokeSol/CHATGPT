...
async function boot(){
  try{
    const [snapshot,national,constituencies,parties,evidence,methodology,daily,editionCurrent,editionIndex]=await Promise.all([
      load('current_snapshot.json'),load('national_projection.json'),load('constituency_cards.json'),load('party_cards.json'),load('evidence_index.json'),load('methodology_state.json'),
...
function renderHeader(){
  ...
  $('#status-detail').textContent=noChange?'Version de référence avant intégration de nouvelles informations 2026 suffisamment confirmées.':'Une version enrichie de la projection est disponible.';
...
function renderHistory(){
  ...
  if(!rows.length){const d=dateFr(D.snapshot.created_at);$('#history-rail').innerHTML=`<article class="history-card-premium current"><span class="date">${d}</span><h3>Projection structurelle de référence</h3><p>Première projection nationale entièrement territorialisée : 395 sièges couverts dans chaque scénario simulé.</p><span class="status">Version enregistrée</span></article><article class="history-card-premium future"><span class="date">Prochaine édition</span><h3>Actualisation quotidienne</h3><p>Les nouvelles informations suffisamment documentées apparaîtront ici sans réécrire les éditions précédentes.</p><span class="status">À venir</span></article>`;return}
