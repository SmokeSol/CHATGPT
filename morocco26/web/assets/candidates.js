(()=>{
  ...
  if(!NAV.some(([id])=>id==='candidates')) NAV.splice(navIndex<0?NAV.length:navIndex,0,['candidates','Candidatures','']);
  ...
  if(main&&!document.querySelector('#view-candidates')){
    ...
    section.innerHTML=`
      <div class="view-head candidates-head"><div><span class="section-kicker">Intelligence territoriale · Candidatures 2026</span><h2>Candidatures documentées par circonscription</h2><p>Un répertoire bilingue consolidant annonces officielles et sources de référence, relié aux 92 circonscriptions locales pour une lecture opérationnelle du terrain électoral.</p></div><div class="edition-badge v1-edition"><span>COUVERTURE DOCUMENTAIRE</span><b id="v1-generated">—</b></div></div>
      <div class="v1-separation" role="note"><div class="v1-separation-mark">395</div><div><b>Projection de référence</b><span>La projection publiée reste distincte de la documentation des candidatures et évolue uniquement lorsque de nouveaux éléments sont suffisamment établis et quantifiables.</span></div><div class="v1-separation-status">RÉFÉRENCE CONSERVÉE</div></div>
  ...
  function renderKpis(){
    ...
    const values=[[c.arabic_identity_coverage||0,'territoires bilingues','hi'],[c.candidate_records_after_official_pjd_merge||0,'fiches candidatures',''],[c.local_territories_with_records_after_merge||0,'circonscriptions couvertes',''],[c.parties||0,'partis documentés',''],[p.filter(r=>r.person_name_ar&&r.person_name_lat).length,'fiches PJD bilingues',''],[p.filter(r=>r.nomination_status==='PENDING_NOMINATION').length,'investitures à confirmer','warn']];
  ...
  function renderDetail(){
    ...
    $('#candidate-detail').innerHTML=`<header class="candidate-detail-head"><div><span class="section-kicker">${esc(t.region_fr)}</span><h3>${esc(t.name_fr)}</h3><div class="candidate-arabic-title" dir="rtl" lang="ar">${esc(t.name_ar)}</div></div><div class="candidate-seat"><b>${t.seats}</b><span>siège${t.seats>1?'s':''}</span></div></header><div class="candidate-detail-stats"><div><b>${cs.length}</b><span>fiches visibles</span></div><div><b>${parties.length}</b><span>partis visibles</span></div><div><b>92/92</b><span>identité territoriale</span></div></div><div class="candidate-no-impact"><b>Lecture documentaire.</b> Les candidatures et investitures sont présentées comme éléments de contexte territorial. La projection n’évolue que lorsque leur effet peut être établi selon une règle explicite et homogène.</div><div class="candidate-cards">${cs.length?cs.map(renderCandidateCard).join(''):'<div class="candidate-empty">Aucune fiche visible avec ces filtres.</div>'}</div>`;
  }
  function renderCandidates(){
    ...
    renderTerritoryList();renderDetail();const status=$('#status-detail');if(status)status.textContent=`Projection territoriale de référence · ${Number(V1.coverage?.candidate_records_after_official_pjd_merge||0).toLocaleString('fr-FR')} fiches candidatures · 92 circonscriptions bilingues.`;
  }
  boot=async function(){await originalBoot();try{V1=await load('atlas_v1.json');D.atlasV1=V1;renderCandidates()}catch(error){console.error('Candidate intelligence unavailable',error);const tab=$('#tab-candidates');if(tab)tab.title='Données candidatures temporairement indisponibles'}};
})();
